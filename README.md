...
| 🗄️ `data-model.md` | `/speckit.plan` | Schema changes (if applicable) |
...
| `/quality` | — | 🛡️ Run comprehensive quality checks (lint, types, tests) |
| `/security-scan` | — | 🔒 Scan staged changes for security issues |
| `/speckit.init` | — | 🏗️ Bootstrap `.specify/` directory in current project |
| `/speckit.constitution` | — | 📜 Create/update project governance principles |
| `/speckit.specify` | `<feature>` | ✍️ Generate spec with scenarios, requirements, success criteria |
...
- 🛡️ **quality-guardian**: Runs quality gate between implementation phases in `/speckit.implement`
