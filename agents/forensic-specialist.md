...
> **Scope**: Use this agent for incident response, threat hunting, malware analysis, and forensic investigations. For routine code security reviews (secrets, SQLi, XSS, auth), use the built-in `/security-review` skill or the `/adf.security-scan` command instead.
